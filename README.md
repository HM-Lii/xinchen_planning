...
当前版本保持主车所在车道，通过纵向QP生成满足速度、acceleration、jerk和本体无碰撞硬约束的轨迹，并以带代价的松弛量尽量满足1.5秒跟随时距。期望距离按实际自车速度计算，过大的距离盈余按6秒时间常数转换为渐进追近速度，冲突时间按真实闭合速度计算。侵入判定只使用邻车当前轮廓，不做横向预测外推；当前轮廓越过车道线后，参考限速随侵入深度从最高限速线性下降，在进入硬碰撞走廊时达到邻车当前纵向速度。周期道路样条与跨周期路径拼接用于保持Cartesian输出连续，运行时提供规划状态和Cartesian运动学双层监控。
